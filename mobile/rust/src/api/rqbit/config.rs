...
pub use  librqbit::{ConnectionOptions, ListenerMode, ListenerOptions, PeerConnectionOptions, dht::PersistentDht, limits::LimitsConfig, WithStatusError};
pub use  serde::{Deserialize, Serialize};
pub use  serde_with::serde_as;

pub use axum::Json;
pub use librqbit::{
    Api, ApiError, Session, SessionOptions, SessionPersistenceConfig,
...
pub type AppDuration = std::time::Duration;
...
pub struct RqbitDesktopConfigConnections {
    ...
    pub listen_port: u16,
     ...
     pub peer_connect_timeout: AppDuration,
     pub peer_read_write_timeout: AppDuration,
}
...
impl Default for RqbitDesktopConfigConnections {
    fn default() -> Self {
        Self {
            ...
            peer_connect_timeout: AppDuration::from_secs(2),
            peer_read_write_timeout: AppDuration::from_secs(10),
        }
    }
}
...
#[derive(Clone)]
pub struct AppState {
    pub shared: Arc<Mutex<Option<SharedAppState>>>,
}

impl AppState {
    pub fn api(&self) -> Result<Api, ApiError> {
      self.shared.lock().unwrap().clone().unwrap().api.with_status_error(StatusCode::FAILED_DEPENDENCY, "not configured")
    }

    pub fn work_dir(&self) -> Option<String> {
        self.shared.lock().unwrap().clone()?.work_dir.clone()
    }

    pub async fn start(&self, work_dir: &str) -> Json<Result<SessionStatsSnapshot, ApiError>> {
        let mut opts = SessionOptions::default();
        let path = PathBuf::from(String::from(work_dir));
        ...
        let session = Session::new_with_opts(work_dir.into(), opts).await.unwrap();
        self.shared.lock().unwrap().replace(SharedAppState {
            work_dir: Some(work_dir.parse().unwrap()),
            api: Some(Api::new(session, None, None)),
        });
